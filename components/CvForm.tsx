...
import { generateSummary, generateDescription, suggestSkills } from '../services/geminiService';
  ...
  const renderSectionControls = (sectionKey: SectionKey, index: number) => {
    ...
    return (
        <div className="absolute top-4 right-4 flex items-center space-x-1">
            <Button variant="ghost" onClick={() => handleSectionOrderChange(index, 'up')} disabled={index === 0} className="!p-1 h-7 w-7"><Icon name="arrow-up" className="w-4 h-4" /></Button>
            <Button variant="ghost" onClick={() => handleSectionOrderChange(index, 'down')} disabled={index === cvData.sectionOrder.length - 1} className="!p-1 h-7 w-7"><Icon name="arrow-down" className="w-4 h-4" /></Button>
            {isOptional && <Button variant="ghost" onClick={() => removeSection(sectionKey)} className="!p-1 h-7 w-7 text-red-500 hover:bg-red-100"><Icon name="trash" className="w-4 h-4"/></Button>}
        </div>
    );
  }
  ...
  const sectionRenderers: { [key in SectionKey]: () => React.ReactNode } = {
    summary: () => (
        <Section title="Professional Summary">
            ...
            <div id="tour-step-2">
                <Button variant="ai" onClick={handleGenerateSummary} loading={aiLoading['summary']} disabled={!jobDescription}>
                    <Icon name="sparkle" /> Tailor Summary to Job
                </Button>
            </div>
        </Section>
    ),
          ...
          cvData.workExperience.map((work, index) => (
            <div key={work.id} className="p-4 border border-[var(--border-primary)] rounded-lg space-y-4 animate-fade-in-slide-down">
               <div className="flex justify-between items-center">
                ...
                <Button variant="ghost" className="!p-1 text-red-500 hover:bg-red-100" onClick={() => removeListItem('workExperience', work.id)}><Icon name="trash" /></Button>
               </div>
               ...
               <Button variant="ai" onClick={() => handleSuggestDescription(work.id, work.jobTitle, index)} disabled={!work.jobTitle || !jobDescription} loading={aiLoading[work.id]}>
                  <Icon name="sparkle" /> Tailor Description
              </Button>
            </div>
          ))
        ) : (
          ...
          cvData.education.map((edu, index) => (
            <div key={edu.id} className="p-4 border border-[var(--border-primary)] rounded-lg space-y-4 animate-fade-in-slide-down">
               <div className="flex justify-between items-center">
                  ...
                  <Button variant="ghost" className="!p-1 text-red-500 hover:bg-red-100" onClick={() => removeListItem('education', edu.id)}><Icon name="trash" /></Button>
               </div>
    ...
    skills: () => (
        <Section title="Skills">
            <Textarea label="Skills (comma-separated)" value={cvData.skills} onChange={(e) => handleFieldChange('skills', e.target.value)} rows={3} />
             <Button variant="ai" onClick={handleSuggestSkills} disabled={!jobDescription} loading={aiLoading['skills']}>
          ...
          cvData.projects.map((project, index) => (
            <div key={project.id} className="p-4 border border-[var(--border-primary)] rounded-lg space-y-4 animate-fade-in-slide-down">
               <div className="flex justify-between items-center">
                  ...
                  <Button variant="ghost" className="!p-1 text-red-500 hover:bg-red-100" onClick={() => removeListItem('projects', project.id)}><Icon name="trash" /></Button>
               </div>
          ...
          cvData.certifications.map((cert, index) => (
            <div key={cert.id} className="p-4 border border-[var(--border-primary)] rounded-lg space-y-4 animate-fade-in-slide-down">
               <div className="flex justify-between items-center">
                  ...
                  <Button variant="ghost" className="!p-1 text-red-500 hover:bg-red-100" onClick={() => removeListItem('certifications', cert.id)}><Icon name="trash" /></Button>
               </div>
        ...
        {cvData.languages.length > 0 ? (
            cvData.languages.map((lang, index) => (
              <div key={index} className="flex items-center space-x-2 animate-fade-in-slide-down">
                 ...
                 <Button variant="ghost" className="!p-1 text-red-500 hover:bg-red-100 mt-1" onClick={() => removeLanguageItem(index)}><Icon name="trash" /></Button>
              </div>
            ))
        ) : (
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 overflow-y-auto h-full">
      <div className="p-6 rounded-lg mb-6" style={{
        ...
        <div className="flex justify-between items-center border-b pb-3 mb-4" style={{ borderColor: 'var(--border-primary)' }}>
          <h2 className="text-xl font-semibold text-[var(--text-primary)]">Controls & Actions</h2>
        ...
        <div className="grid grid-cols-1 gap-4">
            <div id="tour-cover-letter">
                 <Button onClick={() => onGenerateCoverLetter(jobDescription)} variant="primary" className="w-full" disabled={!jobDescription}>
                    <Icon name="sparkle" className="mr-2" /> Generate AI Cover Letter
                </Button>
            </div>
        </div>
      </div>
      ...
      <div id="tour-step-1">
        <Section title="Target Job Description (for AI Tailoring)">
            <Textarea 
            ...
            />
            <p className="text-xs text-[var(--text-secondary)] mt-1">All AI generation features will use this description as their primary context.</p>
        </Section>
      </div>
